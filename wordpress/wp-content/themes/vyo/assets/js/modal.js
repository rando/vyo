...
var Util = function () {
  ...
  var Util = {
    ...
    getSelectorFromElement: function getSelectorFromElement(element) {
      ...
      try {
        var jQueryselector = jQuery(document).find(selector);
        return jQueryselector.length > 0 ? selector : null;
      } catch (error) {
        return null;
      }
    },
...
var Modal = function () {
  ...
  function () {
    function Modal(element, config) {
      ...
      this._dialog = jQuery(element).find(Selector.DIALOG)[0];
      this._backdrop = null;
      this._isShown = false;
    ...
    _proto.show = function show(relatedTarget) {
      ...
      if (Util.supportsTransitionEnd() && jQuery(this._element).hasClass(ClassName.FADE)) {
      ...
      jQuery(document.body).addClass(ClassName.OPEN);

      this._setEscapeEvent();

      this._setResizeEvent();

      jQuery(this._element).on(Event.CLICK_DISMISS, Selector.DATA_DISMISS, function (event) {
    ...
    _proto._getConfig = function _getConfig(config) {
      config = jQuery.extend({}, Default, config);
    ...
    _proto._showElement = function _showElement(relatedTarget) {
      ...
      if (transition) {
        jQuery(this._dialog).one(Util.TRANSITION_END, transitionComplete).emulateTransitionEnd(TRANSITION_DURATION);
      } else {
        transitionComplete();
      }
    };
    ...
    _proto._enforceFocus = function _enforceFocus() {
      ...
      jQuery(document).off(Event.FOCUSIN) // guard against infinite focus loop
      .on(Event.FOCUSIN, function (event) {
        if (document !== event.target && _this4._element !== event.target && !jQuery(_this4._element).has(event.target).length) {
    ...
    _proto._showBackdrop = function _showBackdrop(callback) {
      ...
      if (this._isShown && this._config.backdrop) {
        ...
        if (animate) {
          jQuery(this._backdrop).addClass(animate);
        }
        ...
        if (Util.supportsTransitionEnd() && jQuery(this._element).hasClass(ClassName.FADE)) {
          jQuery(this._backdrop).one(Util.TRANSITION_END, callbackRemove).emulateTransitionEnd(BACKDROP_TRANSITION_DURATION);
        } else {
          callbackRemove();
        }
      } else if (callback) {
    ...
    _proto._setScrollbar = function _setScrollbar() {
      ...
      if (this._isBodyOverflowing) {
        ...
        jQuery(Selector.FIXED_CONTENT).each(function (index, element) {
          var actualPadding = jQuery(element)[0].style.paddingRight;
        ...
        jQuery(Selector.STICKY_CONTENT).each(function (index, element) {
          var actualMargin = jQuery(element)[0].style.marginRight;
        ...
        jQuery(Selector.NAVBAR_TOGGLER).each(function (index, element) {
          var actualMargin = jQuery(element)[0].style.marginRight;
          var calculatedMargin = jQuery(element).css('margin-right');
          jQuery(element).data('margin-right', actualMargin).css('margin-right', parseFloat(calculatedMargin) + _this9._scrollbarWidth + "px");
        }); // Adjust body padding
        ...
        var calculatedPadding = jQuery('body').css('padding-right');
        jQuery('body').data('padding-right', actualPadding).css('padding-right', parseFloat(calculatedPadding) + this._scrollbarWidth + "px");
      }
    };
    ...
    _proto._resetScrollbar = function _resetScrollbar() {
      ...
      jQuery(Selector.FIXED_CONTENT).each(function (index, element) {
        ...
        if (typeof padding !== 'undefined') {
          jQuery(element).css('padding-right', padding).removeData('padding-right');
        }
      }); // Restore sticky content and navbar-toggler margin

      jQuery(Selector.STICKY_CONTENT + ", " + Selector.NAVBAR_TOGGLER).each(function (index, element) {
      ...
      var padding = jQuery('body').data('padding-right');

      if (typeof padding !== 'undefined') {
        jQuery('body').css('padding-right', padding).removeData('padding-right');
      }
    };
    ...
    Modal._jQueryInterface = function _jQueryInterface(config, relatedTarget) {
      return this.each(function () {
        ...
        var _config = jQuery.extend({}, Modal.Default, jQuery(this).data(), typeof config === 'object' && config);
  ...
  jQuery(document).on(Event.CLICK_DATA_API, Selector.DATA_TOGGLE, function (event) {
    ...
    var jQuerytarget = jQuery(target).one(Event.SHOW, function (showEvent) {
      ...
      jQuerytarget.one(Event.HIDDEN, function () {
