	...
	<div class="container tour-detail-fixed ">
		<div class="row pt-35 pb-35">
			...
			<div class="col-12 col-md-2 text-center text-md-left tour-price">
				...
				<span id="tour-price">
				<?php
					the_field('discount-price') 
				?></span><span id="tour-currency"> <?= the_field('currency'); ?></span> 
				<del><?= the_field('price') ?> <?= the_field('currency'); ?></del>
				<?php
				} else {
				?>
					<span id="tour-price"><?php the_field('price') ?> </span><span id="tour-currency"> <?= the_field('currency'); ?> </span>
				<?php
				}
				?>	
			</div>
...
<?php if (get_site_url() != 'http://localhost:8000') { ?>
    <script type="text/javascript">
        jQuery('#tour-order-btn').on('click', function() {
            ga('send', 'event', '', 'Order', 'Go');
        });

        jQuery('#nf-field-4').on('click', function() {
